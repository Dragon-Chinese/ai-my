...
const utcStr = '2025-07-18T16:00:00.000Z'
...
const set_all_model = async () => {
  await store.set_all_model("F5")
};

const get_list_tts = async () => {
  try {
    const response = await store.get_list_tts();
    console.log(response);
    list_tts.value = response;
  } catch (error) {
...
const getConfig = () => {
  store.getConfig().then(res => {
    ...
    musicUrl.value = store.initData.ref_audio_path || '';
...
const musicChange = (val) => {
  const selected = list_tts.value.find(item => item.absolute_path === val);
  musicUrlChange(selected);
}
...
const musicUrlChange = (e) => {
  ...
  audioUrl.value = `http://localhost:3201/api/audio/${e.filename}`
          ...
          <div class="grid-content min-height436">
            ...
            <div class="grid-content-item marginBottom40">
              <div class="grid-content-item-title flex">
                ...
                <div class="buttonBox flex">
                  <el-button color="#3E61FF" @click="get_list_tts()" plain>刷新音频</el-button>
                  <!-- 直接在模板里用 $refs 出发播放 -->
              ...
              <el-select v-model="musicUrl" filterable placeholder="请选择音频文件" @change="musicChange"
                class="custom-select">
                <el-option v-for="item in list_tts" :key="item.absolute_path" :label="item.filename"
            ...
            <div class="grid-content-item">
              <div class="grid-content-item-title flex">
                ...
                <div class="buttonBox flex">
                  <el-button color="#3E61FF" @click="setConfig">
                    设置
                  </el-button>
                </div>
              </div>
        ...
        <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="6">
          <div class="grid-content min-height234">
            <div class="grid-content-item">
              ...
              <div v-if="chatType == 1">
                <el-input v-model="linkRoom" style="height: 50px; margin-bottom: 10px;" placeholder="请输入抖音直播间链接，格式：https://live.douyin.com/288735743xxx" clearable />
                <div class="buttonBox flex">
                  <el-button color="#3E61FF" @click="connect" :disabled="store.initData.liveId == '' ? false : true">
                    连接
                  </el-button>
                  <el-button color="#3E61FF" @click="connectStop"
                    :disabled="store.initData.liveId == '' ? true : false" plain>
                    断开
                  </el-button>
                </div>
              </div>
