    ...
    <router-link v-slot="{ href, navigate }" custom :to="uniqueKey.at(-1) ?? ''">
      <FaTooltip :disabled="!rootMenu.isMenuPopup || level !== 0 || subMenu" :text="typeof item.meta?.title === 'function' ? item.meta?.title() : item.meta?.title" :side="rootMenu.props.mode === 'vertical' ? 'right' : 'bottom'" class="h-full w-full">
        <component
          ...
          }" :class="cn('group menu-item-container relative h-full w-full flex cursor-pointer items-center justify-between gap-1 rounded-lg px-4 py-3 text-[var(--g-sub-sidebar-menu-color)] transition-all hover-(bg-[var(--g-sub-sidebar-menu-hover-bg)] text-[var(--g-sub-sidebar-menu-hover-color)]) subMenu', {
            'text-[var(--g-sub-sidebar-menu-active-color)]! bg-[var(--g-sub-sidebar-menu-active-bg)]! isItemActive': isItemActive,
        ...
        > 
          <div :class="cn('line', {'active': isItemActive})"></div>
          <div
            :class="cn('icon-item inline-flex flex-1 items-center justify-center gap-[8px] pl-[calc(var(--indent-level)*20px)]', {
          ...
          > 
            <FaIcon v-if="props.item.meta?.icon" :name="props.item.meta.icon" class="menu-item-container-icon transition-transform group-hover-scale-120" />
            <span
...
<style lang="scss">
  ...
  .subMenu .menu-item-container-icon {
    width: 14px;
    height: 14px;
  }
  .subMenu .line {
  ...
  .subMenu.isItemActive {
    background: linear-gradient( 90deg, rgba(62,97,255,0.69) 0%, rgba(62,97,255,0.12) 100%) !important;
    .truncate {
      color: #fff;
    }
  }
